...
const modalMonitora = new bootstrap.Modal(document.getElementById('modalMonitora'));
const formMonitora = document.getElementById('formMonitora');
...
const btnCrear = document.getElementById('btnCrearM');
...
// Función para mostrar los resultados solo para el tipo de rol "Monitora"
const mostrar = (monitora) => {
    resultados = ''; // Limpiar resultados anteriores
    // Separar estudiantes activos e inactivos
    const activos = monitora.filter(monitora => monitora.Estado === 'activo');
    const inactivos = monitora.filter(monitora => monitora.Estado === 'inactivo');

    // Combinar las listas
    const todos = [...activos, ...inactivos];
    todos.filter(monitora => monitora.TipoRol === 'Monitora').forEach(monitora => {

        // Usar la clase fila-inactiva si el estado es inactivo
        const filaClass = monitora.Estado === 'inactivo' ? 'fila-inactiva' : ''; // Clase para inactivo
        // Determinar los botones a mostrar
        const btnEditar = monitora.Estado === 'activo' ? '<a class="btnEditar btn btn-primary" style="color: white;">Editar</a>' : '';
        const btnInactivar = monitora.Estado === 'activo' 
        ...
        resultados += `<tr class="${filaClass}">
                            <td>${monitora.IdUsuario}</td>
                            <td>${monitora.Imagen ? `<img src="${monitora.Imagen}" alt="Imagen" width="50" height="50">` : `<img src="img/sinfoto.jpg" alt="Imagen no disponible" width="50" height="50">`}</td>
                            <td>${monitora.Nombres}</td>
                            <td>${monitora.Apellidos}</td>
                            <td>${monitora.TipoIdentificacion}</td>
                            <td>${monitora.NoIdentificacion}</td>
                            <td>${monitora.telefono}</td>
                            <td>${monitora.Correo}</td>
                            <td>${monitora.Usuario ? monitora.Usuario : 'No disponible'}</td>
 ...
 // Función para buscar Monitoras
 const buscarMonitoras = (query) => {
...
// Mostrar los usuarios tipo "Monitora" al cargar la página
fetch(url)
    .then(response => response.json())
    .then(data => mostrar(data))
    .catch(error => console.log(error));
...
on(document, 'click', '.btnInactivar', e => {
    ...
    Swal.fire({
        title: "Inactivar Monitor/a",
        text: "¿Estás seguro de que deseas inactivar este monitor/a?",
        icon: "warning",
        ...
        if (result.isConfirmed) {
            ...
            fetch(`${url}/inactivar/${id}`, { method: 'PUT' })
                ...
                .then(response => {
                    ...
                    Swal.fire({
                        title: "¡Inactivado!",
                        text: "Monitor/a inactivado con éxito.",
                        icon: "success"
                    });
                })
                .catch(error => {
                    console.error('Error al inactivar el monitor/a:', error);
                    // Mostrar swal de error
                    Swal.fire({
                        title: "Error",
                        text: "Error al inactivar el monitor/a.",
                        icon: "error"
                    });
                })
                .finally(() => {
...
on(document, 'click', '.btnActivar', e => {
    ...
    Swal.fire({
        title: "Activar Monitor/a",
        text: "¿Estás seguro de que deseas activar este monitor/a?",
        icon: "warning",
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            fetch(`${url}/activar/${id}`, { method: 'PUT' })
                ...
                .then(response => {
                    ...
                    Swal.fire({
                        title: "¡Activado!",
                        text: "Monitor/a activado con éxito.",
                        icon: "success"
                    });
                })
                .catch(error => {
                    console.error('Error al activar el monitor/a:', error);
                    alertify.error('Error al activar el monitor/a');
                })
                .finally(() => {
...
formMonitora.addEventListener('submit', (e) => {
    e.preventDefault();
    ...
    Swal.fire({
        title: "Confirmación",
        text: opcion === 'crear' ? "¿Estás seguro de que deseas crear este Monitora?" : "¿Estás seguro de que deseas actualizar este Monitora?",
        icon: "warning",
        showCancelButton: true,
            ...
            fetch(requestUrl, requestOptions)
                ...
                .then(data => {
                    ...
                    Swal.fire(opcion === 'crear' ? 'Creado' : 'Actualizado', 
                              `Monitora ${opcion === 'crear' ? 'creado' : 'actualizado'} con éxito`, 
                              'success');
                })
                .catch(error => {
                    console.log(error);
                    Swal.fire('Error', `Error al ${opcion === 'crear' ? 'crear' : 'actualizar'} el Monitora`, 'error');
                });
